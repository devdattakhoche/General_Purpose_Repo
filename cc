...
USE reference // DROP PROCEDURE IF EXISTS reference.sp_process_grips_data // CREATE PROCEDURE reference.sp_process_grips_data(IN par_delete_missing BIT) 
BEGIN
...
SELECT
   ...
   clientBookId,
   clientTradingUnit,
   clientAccountMnemonic,
   flowType,
   productType,
   exchangeIds 
FROM
   reference.grips_data_raw 
WHERE
   flowType IN 
   (
      'DMA',
...
CREATE TEMPORARY TABLE IF NOT EXISTS ` # tempSplitExchangeIds` ( exchangeId VARCHAR(20) NULL );
-- Debug print statement
SELECT
   CONCAT_WS( '', 'Delete mode: ', CAST(par_delete_missing AS CHAR) ) INTO var_print_string;
...
IF (par_delete_missing = 1) 
THEN
   call reference.sp_log_message( 'sp_process_grips_data', 'Is deleted mode true' );
UPDATE
   reference.client_preference 
SET
   last_modified_by = CONCAT('x', last_modified_by) 
WHERE
   exec_region = 'AP' 
   AND pref_type IN 
   (
      '&BB', 'IBB', '+LB', '+SB', 'ELB', 'ESB' 
   )
...
OPEN get_grips_row;
-- Outer loop
outer_loop : LOOP FETCH get_grips_row INTO var_cass_id, var_sub_cass_id, var_countryCode, var_longBookId, var_shortBookId, var_longEtfBookId, var_shortEtfBookId, var_washBookId, var_clientBookId, var_clientTradingUnit, var_clientAccountMnemonic, var_flowType, var_productType, var_gripsExchangeIds;
IF done_outer 
THEN
   LEAVE outer_loop;
END
...
FROM
   reference.client 
WHERE
   id_cass2 = var_cass_id 
   AND is_migrated = 'Y' into var_count;
IF var_count = 0 
THEN
   SELECT
      CONCAT_WS( '', 'cass2 not setup, cass2=', CAST( var_cass_id AS CHAR (12) ) ) INTO var_print_string;
call reference.sp_log_message( 'sp_process_grips_data', var_print_string );
ITERATE outer_loop;
END
IF;
SELECT
   CONCAT_WS( '', 'Outer Loop where cass_id = ', 
   CASE
      WHEN
         var_cass_id IS NULL 
      THEN
         'NULL' 
      ELSE
         CAST( var_cass_id AS CHAR(10) ) 
   END
, ' and country_code = "', var_countryCode, '" and long_book_id = "', var_longBookId, '" and short_book_id = "', var_shortBookId, '" and long_etf_book_id = "', var_longEtfBookId, '" and short_etf_book_id = "', var_shortEtfBookId, '" and wash_book_id = "', var_washBookId, '" and client_book_id = "', var_clientBookId, '" and client_trading_unit = "', var_clientTradingUnit, '" and client_account_mnemonic = "', var_clientAccountMnemonic, '" and flow_type = "', var_flowType, '" and product_type = "', var_productType, '" and grips_exchange_ids = "', var_gripsExchangeIds, '"' ) INTO var_print_string;
CALL reference.sp_log_message( 'sp_process_grips_data', var_print_string );
-- Process based on countryCode
IF (var_countryCode = 'CN') 
THEN
   -- Example modification to var_gripsExchangeIds
   IF ( LOCATE('[', var_gripsExchangeIds) > 0 ) 
THEN
   SELECT
      SUBSTRING( var_gripsExchangeIds, LOCATE('[', var_gripsExchangeIds) + 1, LOCATE(']', var_gripsExchangeIds) - 2 ) INTO var_gripsExchangeIds;
END
IF;
-- Splitting var_gripsExchangeIds
SELECT
   LOCATE(',', var_gripsExchangeIds) INTO var_pos;
IF (var_pos = 0) 
THEN
...
WHILE ( var_pos <> 0 
AND var_iterations < 20 ) DO 
SELECT
   SUBSTRING( var_gripsExchangeIds, 1, var_pos - 1 ) INTO var_exchId;
INSERT INTO
   ` # tempSplitExchangeIds` 
VALUES
   (
...
;
SET
   var_gripsExchangeIds = INSERT( var_gripsExchangeIds, 1, var_pos, NULL );
SELECT
   LOCATE(',', var_gripsExchangeIds) INTO var_pos;
SET
   var_iterations = var_iterations + 1;
END
WHILE;
...
set
   done_inner = 0;
-- Inner loop
inner_loop : LOOP FETCH get_grips_exchangeid_row INTO var_gripsExchangeId;
IF done_inner 
THEN
set
   done_outer = 0;
...
IF ( (var_flowType = 'NO_TOUCH') 
AND var_productType = 'SWAP' ) 
THEN
   -- Example procedure calls
   IF ( LEFT(var_clientAccountMnemonic, 5) = 'JDMA_' ) 
THEN
SET
   var_clientAccountMnemonic = NULL;
END
IF;
...
UPDATE
   reference.client_preference 
SET
   pref_enabled = 0, last_modified_by = 'dGRIPS', last_modified_date = NOW(6) 
WHERE
   exec_region = 'AP' 
   AND pref_type IN 
   (
